...
casper.test.begin('UserLogin',function UserLogin(test)
{
    ...
    {
        //Reload page as sometimes page doesnt load correctly
        this.reload();
...
casper.test.begin("Nav Spot Market --> "+PageChange+"",function (test)
{
    ...
    casper.waitForSelector("nav ul li[class='"+NavLocation+"']",
        function success()
        {
            console.log('Checking if Nav Ul Li Exists');
            //Checking that Nav Ul Li Exists on Page
            test.assertExists("nav ul li[class='"+NavLocation+"']", "TEST1 --Nav Ul Li Exists");
    ...
    casper.waitForSelector("a[href='"+PageChange+"'] i",
        ...
        function fail()
        {
            console.log("TEST2 --href "+PageChange+" Does Not Exists");
